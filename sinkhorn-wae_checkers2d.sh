...
iteration_target=5000

for bs in 100
do
for lr in 0.0001
do
for wae_lambda in 0.0001
do
...
CUDA_VISIBLE_DEVICES=1 python run.py --pz=${pz} --train_size=${train_size} \
    ...
    --recalculate_size=${recalculate_size} --nat_size=${nat_size} --nat_resampling=${nat_resampling} --tag="syn,syn_2c,local-sinkhorn" \

...
name="celeba_ours"
python run.py --pz=${pz} --sinkhorn_sparse=False --sinkhorn_sparsifier=None  --nat_sparse_indices_num=1000  --enc_noise=deterministic --name="${name}" --nat_size=2000 --nat_resampling=${nat_resampling} --tag="spherical,ordered-blocks,mnist" --exp=${exp} --rec_lambda=${rec_lambda} --wae_lambda=${wae_lambda} --epoch_num=${epoch_num} --ot_lambda=${ot_lambda} --sinkhorn_epsilon=${sinkhorn_epsilon} --sinkhorn_iters=${sinkhorn_iters} --zdim=${zdim} --e_pretrain=False --work_dir=out/sinkhorn_${name}  > out/sinkhorn_${name}.cout 2> out/sinkhorn_${name}.cerr

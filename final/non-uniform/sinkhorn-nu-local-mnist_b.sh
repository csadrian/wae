...
CUDA_VISIBLE_DEVICES=7 python run.py --pz=${pz} --train_size=${train_size} \
    --z_test=${z_test} --z_test_scope=${z_test_scope} \
    --sinkhorn_sparse=False --enc_noise=deterministic --name="${name}" \
    --recalculate_size=${recalculate_size} --nat_size=${nat_size} --nat_resampling=${nat_resampling} --tag="mnist-nonshuffled,local-sinkhorn" \

...
class WAE(object):
    ...
    def add_nat_placeholders(self):
        ...
        self.grad_ph = tf.placeholder(shape=(opts['nat_size'], opts['zdim']), dtype=tf.float32)
    ...
    def matching_penalty(self):
        ...
        if opts['z_test'] == 'gan':
            ...
            loss_match = improved_wae.mmdpp_1d_penalty(
                opts, self, sample_pz)
        elif opts['z_test'] == 'sinkhorn_new':
            @tf.custom_gradient
            def inject_grad_layer(x):
              def grad(dy):
                return self.grad_ph
    ...
    def my_sinkhorn_grad_init(self):

        block_size = self.opts['sinkhorn_block_size']
        epsilon = self.opts['sinkhorn_epsilon']
        ...
        self.partial_dist_t = sinkhorn.pdist(self.x_latents, tf.stop_gradient(self.partial_ys))
    ...
    def my_sinkhorn_grad(self, feed_d):
        block_size = self.opts['sinkhorn_block_size']
        block_iters = self.train_size // block_size
        self.sess.run([self.assign_sinkhorn_f, self.assign_sinkhorn_g], feed_dict={ self.assign_inp: np.zeros(self.train_size) })
          ...
          for epoch in range(opts["epoch_num"]):
            ...
            for it in range(batches_num):
                ...
                if counter % opts['print_every'] == 0:
                    ...
                    feed_d_loss = {self.latents_ph: test_latents,
                                   self.targets_ph: test_targets,
                                   self.is_training: False }
                    if self.sparsifier is not None:
                        if not hasattr(self, 'indices_full'):
                            self.indices_full = np.array([k for k in itertools.product(np.arange(self.num_pics, dtype=np.int64), repeat=2)])
                    ...
                    feed_d_test = {}
                    feed_d_test[self.sample_points] = data.data[:self.num_pics]
                    feed_d_test[self.is_training] = False
                    if self.sparsifier is not None:
                        feed_d_test[self.nat_sparse_indices] = self.indices_full
                    [loss_rec_train, enc_train, rec_train] = self.sess.run([self.loss_reconstruct, self.encoded, self.reconstructed], feed_dict=feed_d_test)
